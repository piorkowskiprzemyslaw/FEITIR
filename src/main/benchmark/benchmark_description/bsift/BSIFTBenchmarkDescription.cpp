...
namespace feitir {
    ...
    const JSONObject::string &BSIFTBenchmarkDescription::getDatabasePath() const {
        return databasePath;
    }

    const JSONObject::string &BSIFTBenchmarkDescription::getVocabularyPath() const {
        return vocabularyPath;
    }

    const JSONObject::string &BSIFTBenchmarkDescription::getVocabularyType() const {
        return vocabularyType;
    }

    JSONObject::boolean BSIFTBenchmarkDescription::isMeasureTime() const {
        return measureTime;
    }

    const JSONObject::string &BSIFTBenchmarkDescription::getResultFile() const {
        return resultFile;
    }
    ...
    const std::vector<JSONObject::number> & BSIFTBenchmarkDescription::getASet() const {
        return aSet;
    }

    const std::vector<JSONObject::number> & BSIFTBenchmarkDescription::getBSet() const {
        return bSet;
    }
    ...
    void BSIFTBenchmarkDescription::setBasicFields(const JSONObject::ValuesMap &valuesMap) {
        ...
        if (valuesMap.count(FIELD_NAMES[3])) measureTime = boost::lexical_cast<JSONObject::boolean>(valuesMap.at(FIELD_NAMES[3]));
        if (valuesMap.count(FIELD_NAMES[4])) resultFile = valuesMap.at(FIELD_NAMES[4]);
        if (valuesMap.count(FIELD_NAMES[5])) aSet = Util::fileToElements<JSONObject::number>(valuesMap.at(FIELD_NAMES[5]));
        if (valuesMap.count(FIELD_NAMES[6])) bSet = Util::fileToElements<JSONObject::number>(valuesMap.at(FIELD_NAMES[6]));
    }
