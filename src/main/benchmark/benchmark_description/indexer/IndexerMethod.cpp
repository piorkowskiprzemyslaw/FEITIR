...
namespace feitir {
    ...
    void IndexerMethod::setBasicFields(const JSONObject::ValuesMap &valuesMap) {
        ...
        if (valuesMap.count(FIELD_NAMES[4])) treshold = boost::lexical_cast<number>(valuesMap.at(FIELD_NAMES[4]));
        ...
        if (valuesMap.count(FIELD_NAMES[8])) codeWordSize = boost::lexical_cast<number>(valuesMap.at(FIELD_NAMES[8]));
    }
